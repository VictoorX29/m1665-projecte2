...
async function guardarResposta() {
	...
	try {
		...
		if (typeof insertResposta === 'function') {
			await insertResposta({ grup, puntuacio, comentari, data: now });
			await carregarRespostes();
		} else {
			// Fallback local push if supabase not configured
			const nova = { id: Date.now(), grup, puntuacio, comentari, data: now };
			respostes.push(nova);
		}
